...
> 本节讲**判定内核**，渲染在下一节；两层各有自己的开关，只开内核时页面不会有任何变化。
...
**UI 入口（v6.6 补齐）**：设置面板 →「全局」页签 →**「区域反色（部分反色）」区块** ——
12 个 `region*` 开关/滑块全部落在这里，另含只读诊断行（分割次数 / 平均耗时 / 降级次数分原因 /
缓存命中率 / 覆盖层占用）与「清空纠正数据」「回滚面积门」「进入区域纠正模式」三个入口。

> 该区块此前**不存在**：v6-1/2/3 交付了内核、渲染层与数据回路，但整套开关从未落到面板上，
> 用户只能改控制台偏好。v6.6 的 UI 重建按 v6-3 PRD 已列明的面板项把它们补齐。

---

## 🧪 v6.0 开发中 (自动部分反色 · 渲染层已落地)

> **状态：渲染层已落地，但整套功能仍然默认关闭** —— 打开 `regionSegment` 与 `regionRender`
> 之后才会看到「白底幻灯片里的彩色插图保持原色、其余反色」的效果。

渲染层用 **`backdrop-filter` 覆盖层**在合成器层反色，不读像素 —— 所以跨域图、跨域视频、
DRM 视频同样能反色（克隆层方案在这些场景会直接失败）。

- **覆盖层是独立 DOM 层**，不挂到媒体元素自身（媒体是 replaced element，不生成伪元素），
  也不借用站点包裹元素的 `::after`（几何对不齐且会与站点自己的伪元素打架）。
- **掩码贴合内容盒**：`object-fit: contain` 的 letterbox 边距、被祖先 `transform` 放大的元素，
  掩码都跟着对齐；边距区绝不反色。
- **位图与矢量两种表达渲染结果像素级等价**（浏览器 bench 用真实截图逐像素比对过）。
- **覆盖层绝不挡点击**（`pointer-events: none` + 命中测试穿透），不进入无障碍树，
  `z-index` 取最小可用值。
- **采样不到画面时显式降级**：祖先带 `filter` / 半透明 / 混合模式 / 自身也是 backdrop-filter
  都会让覆盖层采样不到 → 退回整图反色，并发一条中文提示（每个原因每会话一次），
  **绝不静默失效**。
- **全屏 / 画中画**：进出时暂停与恢复部分反色（覆盖层进不了全屏顶层），有提示可解释。
- **视频 / GIF**：掩码按心跳（默认 1s，可调）与场景跃变重算，帧间沿用旧掩码 ——
  所以**场景渐变时"哪块该反"会滞后**（反色本身仍是实时的）；不想要这个代价可以开「仅静态图」。
- **元素被移除 / 页面切走 / 关掉开关**：覆盖层与掩码定义一并回收，无残留。

默认关闭时零覆盖层、零新增节点 —— 29 个既有浏览器端到端场景零回归。

---

## 🧪 v6.0 开发中 (自动部分反色 · 纠正与自校准已落地)

> **状态：默认关闭。** 这是一个**可选**的数据通道，不开也完全可用（跑默认参数）。

部分反色的区域是**自动分割**出来的（不存在手动划选入口）。分割参数是拍的，缺一个监督信号。
这一片补的就是那一环：**点一下 = 把一个区域的判定取反**。

- **唯一操作是点击**：进入「区域纠正」后，媒体上会叠一层可视化（红=反色、蓝=保持原色），
  点哪个区域就翻哪个区域 —— 没有套索、没有笔刷、没有拖顶点。
- **每次纠正产出一份掩码差分样本**（我们算出的区域 vs 你纠正后的区域），只存本地，
  **零网络出口**；可在面板查看与清空，也能随规则包手动导出。
- **自校准是保守且可回滚的**：样本攒够才评估、单次调整有幅度上限、方向不一致就不动、
  只动「最小连通域面积门」这一个参数（**绝不碰整图判定的阈值**），随时一键回滚默认值。
- **不施压**：不提示、不弹窗、不加徽标催你去纠正；纠正数据属于你的个人数据，
  不进仓库、不进日志、不上报。
- 已知局限（如实说明）：纠正按「站点 + 元素词干 + 图片尺寸」记住，**不含内容指纹** ——
  同站不同版式的图可能不命中，此时纠正只在本次会话生效。
