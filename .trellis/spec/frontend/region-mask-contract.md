...
## 7. 差分样本与自校准（v6-3 落地，2026-09-26）

纠正**不是**绘制入口（D1 排除），而是**数据采集通道**：一次点击 = 把一个区域判定取反。
产物是分割器唯一缺的监督信号 —— **掩码差分样本**：

```
样本 = { host, stem, dims: 'WxH', n,
         ours: '0101…'（我们算出的网格，位串）,
         corrected: '0101…'（用户纠正后的网格）,
         diff  = ours XOR corrected, at }
```

- **纠正状态用「翻转集」表达**：`corrected = ours XOR flips`，`diff === flips`。
  连通域**必须在自动结果上取**（不是在纠正后的网格上）——否则第一次翻转后区域与背景合并，
  再点会翻转整张图。再点同一区域 = 翻转集归零 = 翻回原位（此时**删除该键的持久样本**，
  否则纠正会被"粘"回来）。
- **持久化**：`Store` 键 `svi:regionCorrections`；泛化键 = `host + selectorStem + 固有尺寸`；
  上限 200 条（FIFO）、单条 ≤2KB；损坏数据降级为空集（绝不崩）。
  **不引入内容指纹**（已知局限：同站不同版式可能不命中 → 回退到仅本会话生效）。
- **自校准**：只动 `regionMinAreaRatio` 一个参数，四道护栏 ——
  累积门（样本数阈值）、方向一致性（≥2/3）、单次幅度上限、结果钳制，
  外加**一键回滚**到 `REGION_DEFAULTS.minAreaRatio`（单一真源）。
  校准**只影响分割参数**，绝不触碰整图判定阈值（那是 v5-3 的回路）。
- **零遥测**：样本只进本地 `Store`；进规则包是**手动导出**，不新增任何网络出口。
- **可选、无施压**：不进入纠正模式也完全可用（跑默认参数）；不出现任何催纠正的文案/徽标。

## 8. 证据位置

- 单测：`test.js` 的「v6.0 unit tests」「v6.0 阶段 1/2/3/4/5/6/7」块（含 bench 与反例留档）
- 端到端：`test-browser.js` **Scenario 30 / 31**（渲染层：像素等价 / letterbox / 降级 / 全屏 /
  零残留；纠正层：一次点击翻转 / 差分入库 / 拖动无效果 / 退出保留 / 关开关全清）；
  其余 29 场景在默认关闭下零回归
