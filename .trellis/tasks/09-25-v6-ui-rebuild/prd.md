...
> **切片（用户 2026-09-26 裁决）**：拆两轮 —— 第一轮「机制层」（本轮已交付），
> 第二轮「重建层」。下列勾选标注了归属轮次。
> **配色方向（用户 2026-09-26 裁决）**：照搬 Dark Reader 深青调（PRD R7 表逐值照搬）。

- [x] token 单一真源 + 构建注入 + 三处逐字节一致单测 —— **第一轮已交付**
      （真源在用户脚本 `:root` 的 `v6.4-TOKENS-START/END` 块；构建注入 popup/options；
      单测逐字节比对三处 + 断言 token 块之外零颜色字面量）
- [x] 同源控件库落地，旧 `ui` 11 个工厂方法全部收口，无第二套实现 —— **第一轮已交付**
      （搬进 `SviControls`，词汇表 20 项；`ui` 退化为零节点构造的转发垫片；单测断言每个工厂
      在真源里只有一处实现）
- [x] popup 三 tab 可用，既有三条消息协议不破 —— **第二轮 R0 已交付**
      （`Filter` / `Site list` / `More`；既有三条协议只做加法，新增第四条 `svi-site-reset`；
      `test-extension.js` 场景 6 逐条断言往返与真实效果）
- [x] options 页承载全部设置项，清单逐项核对无缺失 —— **第二轮 R1 已交付**（附例外，见下）
      （10 组 / **91 项**全部落页面：`test-extension.js` 场景 7 与**真源 schema 逐键比对**；
      清单单测三向核对 schema ⟷ DEFAULT_PREFS ⟷ 面板键面，未上页面的键在**例外表**里逐条写明理由；
      偏离：面板 3 个区块用**自建 DOM**（原色屏蔽色卡 / 元素规则列表 / 背景排除）与
      全局默认 `bgReplace`，抽出器抓不到 —— 需要新控件类型，登记为 R2 同一批工作）
- [ ] 内嵌面板 12 区块重建，设置项清单与重建前逐项一致 —— **第二轮**
      （R1 已顺带补齐：面板 CSS 里 6 组「v6.4 控件词汇」（chip / collapsible / msg / btn /
      快捷键输入）此前**没有样式**，是 R2 重建的前置）
- [ ] emoji 计数为 0（三处路径）—— **第二轮**（与文案重写合并做，避免同一批字符串改两遍；
      第一轮已实测：27 种字符 / 317 个实例）
- [x] 视觉方向在评审确认，token 值写入 spec —— **第一轮已交付**
      （用户选定 DR 深青调；token 表写入 `.trellis/spec/frontend/ui-design-tokens.md`）
- [x] 功能零回归：四绿门禁全绿，bench 面板相关断言全部通过 —— **第一轮已交付**
      （31 场景 100%；本轮不改 DOM 结构，故未新增/改写任何 bench 断言）
- [ ] 文档：README / README_EN 更新为新 UI 结构说明 —— **第二轮**（本轮无可描述的 UI 变化）
