...
> **切片（本轮编排，2026-09-26）**：R2 是整片里最大的一块，拆成 5 个子阶段、各自一次提交
> （便于单独回滚）。**每个子阶段都跑满五绿门禁**，不用「最后一起跑」。
> 勘察实测（2026-09-26）：面板 12 区块共 **118 处** `ui.*` 调用点（11 个工厂名，全部命中垫片导出表）；
> 垫片之外**只剩 3 个**自建 DOM 的设置（`shieldColors` / `elementRules` / `bgReplace`）需要**新控件类型**；
> 面板 CSS 已于第一轮 M1 token 化（本轮复查：token 块之外仅剩防闪光 `#000` 一处，与单测白名单一致）。

### R2a — 面板调用点切 `SviControls` + 删转发垫片 ✅ 完成
- [x] 118 处 `ui.<工厂>(` → `SviControls.<工厂>(`（11 个工厂名，逐一核对无漏；脚本替换计数 **118**，
      与勘察数逐字相符）
- [x] 删掉 `const ui = {...}` 垫片；改动面板区块 banner 注释与 `ui` 相关注释
- [x] `test.js` 的垫片单测**反向改写**（原断言「垫片必须存在」→「垫片必须不存在 + `ui.*` 调用点为 0」）
- [x] 门禁：`node --check` ✓ / `test.js` ✓ / `test-browser.js` **33 场景** ✓ /
      `test-extension.js` **8 场景** ✓ / build+pack（zip 12 entries）✓
- [x] 判据达标：**bench 断言一条都没改**（DOM 零变化 —— 垫片本就转发到同一实现点），
      33 + 8 场景全绿即证；变红者为 0，故无「改写断言」事项

**R2a 实测到的两个真东西**

1. **垫片之外还有一处隐藏消费者**：`window.__svi` 的调试导出表里有 `uiBuilders: ui` —— 删除垫片后
   `node test.js` 立刻 `ReferenceError: ui is not defined`。改指 `SviControls`（语义未变，原本就是
   转发到同一实现点），**键名保留不改**（它是对外调试句柄，改名属破坏性变更）。
   教训：删一个「只是转发」的符号前，先全仓（含导出表与调试句柄）搜一遍它被谁提到。
2. 垫片单测原先是**正向**断言（「必须存在垫片」）—— 这类断言会随实现演进而变成绊脚石。
   已改成**反向**断言（垫片必须不存在 + 零残留调用点），与「控件只有一个实现点」的纪律同向。


### R2b — 补齐新控件类型 + 三项「自建 DOM」设置进 schema ✅ 完成
- [x] 新增 `SviControls.colorList`（色卡列表增删）与 `SviControls.listEditor`（结构化列表增删）；
      另提前引入 `SviControls.icon`（内联 SVG + `currentColor`）—— 它是 R2d「emoji 清零」的**唯一图标实现点**，
      新控件当场就用它，避免同一处图标改两遍
- [x] 面板 `buildShieldSection` 色卡区 / 元素规则编辑器改用新工厂（DOM 结构随之变化 →
      测试侧撞到的两处按新结构改写，见偏离 15/16/17；**未放宽任何断言**）
- [x] schema 补 `shieldColors`（新分组 `shield` 原色屏蔽）/ `elementRules`（`site` 组）两项
- [x] `test.js` 例外表由 27 条降为 **25 条**（这两条已上页面，表自带断言会逼着删）
- [x] options 渲染层 `KINDS` 增加两种 kind 的实现（不认识的 kind 仍显式报错，不静默跳过）
- [x] 判据达标：options 项数 91 → **93**（10 组 → **11 组**）；`test-extension.js` 场景 7
      与真源逐键比对通过，并新增「复合控件自带件按 schema 字段形状逐项核对」+「复合控件真的能写、
      内容脚本真的能吃到」两组断言
- [x] ~~`bgReplace` 进 schema~~ —— **未做，如实回退**（见偏离 15）

**R2b 实测到的三个真东西**

1. **「面板有 UI」与「设置项该不该上设置页」不是同一件事**：`bgReplace` 的全局默认在面板里
   根本没有独立行（唯一入口是胶囊按钮的站点三态，写的是 `siteOverrides`），而 R1b 的双向断言
   要求「schema 的键必须在面板里也有落点」。硬上设置页就是造出面板没有的设置项 —— 属既有 IA 缺口，
   不属 UI 重建范围，故**回退并保留例外登记**，不擅自扩范围（偏离 7）。
2. **复合控件会打破「全局控件计数」这类断言**：`listEditor` 的添加表单本身就是 2 个下拉 + 1 个文本框，
   于是「下拉数 == select + hour 项数」当场变红。修法不是放宽，而是**按真源判定归属**：
   用 schema 里 `kind ∈ {colorList, listEditor}` 的键圈出复合行，把「位于复合行内部」的元素排除出全局计数，
   再**补一组更严的断言**（每个复合控件里到底有几个什么控件，按 `fields` 声明逐项比对）。
3. **老断言里的隐藏竞态**：场景 5 只等 `htmlClass` 就取 `prefs.hoverRestore`，而 Store 的远端命名空间
   是**异步**装载的（`onRemoteLoaded` 里才 `state = loadState()`）—— 引导期先按默认值渲染出 htmlClass，
   于是变成一个随机红的计时器（本轮复现一次）。修法同样是收紧前置条件（等 `Store._remoteLoaded`），
   不是放宽断言。


### R2c — 胶囊 / toast / 模态 / 区域框选层纳入 token ✅ 完成
- [x] 复核四者 + 全文件面板 UI 注入点的颜色字面量：token 块之外共 **27 处** `#hex`
- [x] 判据：**不是「零改动」** —— 逐处判定后分两类，两类都在代码里写明理由：
      | 类别 | 处数 | 处理 |
      | :--- | ---: | :--- |
      | 数据（色卡表 / 判白比较 / 默认存储值 / 预设示意色） | 24 | 保留字面量 + 逐处写清「为什么不是主题色」 |
      | **主题漂移（本轮修）** | **3** | 改为 token 引用 |
- [x] 修掉的两处漂移：① `MASK_PRESETS.color` 写死 `#0f172a`（旧配色时代的深板岩，换 DR 深青调后
      与面板底色已不同源）→ `var(--svi-bg-deep)`；② 区域纠正可视化层的 SVG 着色写死 `#ef4444`/`#3b82f6`
      —— **同一层的图例早就用 token 了**，只有着色没跟上 → 改走 `--svi-error-bright` / `--svi-fg`
- [x] 踩到的坑（已写成断言守住）：**SVG 表现属性不接受 `var()`**（`fill="var(...)"` 会被当非法颜色丢掉），
      必须落在 `style` 属性上；单测新增断言禁止再写回 `setAttribute('fill', …)`
- [x] 断言策略：第 5 条只覆盖「内联 style 里 `color/background: #hex`」这一种写法，
      对「存进 JS 对象再经 `setProperty`/`setAttribute` 注入」的写法抓不到 —— 新增第 6 条**按已知注入点
      逐处点名核对**（不做全文件扫：会被 24 处数据色淹成噪声，最后只能靠放宽维持绿）

### R2d — 面板 emoji 改内联 SVG ⬜
- [ ] 面板可见文本里的 emoji 全部换成内联 SVG（`fill: currentColor`）；含区块标题、按钮、toast 文案
- [ ] 判据：面板区间 emoji 实例数为 0（本轮实测面板 JS 区间 **70 实例 / 21 种字符**；
      面板 CSS 区间 7 个 `→`，属注释文本不属图标，按 R3 的码位白名单处理）
- [ ] 说明：**吃掉 R3 的面板那部分**，避免同一批文案改两遍（PRD 已预告此合并意图）

### R2e — 跨界面偏好同步修复（用户 2026-09-26 裁决：加订阅 + 版本仲裁）⬜
- [ ] 内容脚本加 `chrome.storage.onChanged` 订阅；用 Store 已有的 `.meta` 版本号做新旧仲裁
- [ ] 收紧 `pagehide` / `visibilitychange` 的**整份回写**（旧版本不回写，消除「旧页覆盖新值」）
- [ ] 新增 bench 场景：「A 页改 → B 页存活 → B 页回写后新值仍在」
- [ ] 判据：上轮实测的复现步骤（options 写 13 → 旧页 pagehide 回写 8 → 新值被覆盖）不再复现
...
**偏离 13（第二轮 · 阶段 R2a）— 删「只是转发」的垫片时，导出表里还有一个隐藏消费者。**

原计划：`ui` 只是转发垫片，把 118 处调用点切到 `SviControls.*` 后删掉它即可。

实际：删掉后 `node test.js` 立刻 `ReferenceError: ui is not defined` —— `window.__svi` 的调试导出表里
写着 `uiBuilders: ui`。改指 `SviControls`（语义未变，原本就是转发到同一实现点），**键名保留不改**
（它是对外调试句柄，改名属破坏性变更）。
教训：删一个「只是转发」的符号前，先全仓（**含导出表与调试句柄**）搜一遍它被谁提到 ——
`grep` 调用点只能找到语法上的用法，找不到被当成值传递的地方。

**偏离 14（第二轮 · 阶段 R2a）— 垫片单测从正向断言改反向。**

原先的单测是「**必须存在** ui 兼容垫片 + 它自己不得 createElement」。这套断言的寿命只到「重建层切换」
为止 —— 切换完成后它会反过来阻止删除垫片。已改为「垫片**必须不存在** + `ui.*` 调用点残留为 0」，
与「控件只有一个实现点」的纪律同向。

**偏离 15（第二轮 · 阶段 R2b）— `bgReplace` 未进 schema（如实回退，不擅自扩范围）。**

原计划（本阶段自己的清单里）：把 `shieldColors` / `elementRules` / `bgReplace` 三项一起进 schema。

实际：前两项顺利；`bgReplace` 当场撞红 `test.js` 的双向对称断言 ——
「schema 的键必须在面板里也有落点」，而该键在面板里**没有任何独立行**（唯一入口是胶囊按钮
「背景:开/关」的三态，写的是 `siteOverrides`，不是这个全局键）。硬上设置页就等于**造出一个面板没有的
设置项**，违反 R1b 建立的对称纪律。
判定：这是既有 IA 缺口（全局默认只能靠规则包 / 备份导入改），**不属 UI 重建范围**，
故回退该项并在 `test.js` 例外表里**如实保留登记**，不在此处擅自扩大改动面。

**偏离 16（第二轮 · 阶段 R2b）— 复合控件打破了「全局控件计数」类断言，改法是收紧而非放宽。**

`test-extension.js` 场景 7 原先按「全局同类元素个数」核对渲染（下拉数 == select + hour 项数）。
`listEditor` 的添加表单本身就是 2 个下拉 + 1 个文本框、`colorList` 自带 1 个取色器，于是这些等式当场变红。

改法（两步，都**加强**断言）：
1. 全局计数按**真源**判定归属：用 schema 里 `kind ∈ {colorList, listEditor}` 的键圈出复合行，
   把位于复合行内部的元素排除出全局计数（不写死类名，不写死数量）；
2. **新增一组更严的断言**：每个复合控件内部到底有几个什么控件，按 schema 的 `fields` 声明逐项比对
   （下拉 / 文本框 / 取色器 / 添加按钮文案），并新增「复合控件真的能写 → 落盘 → 内容脚本真的能吃到」
   的端到端往返（色卡探针色 + 规则探针选择器，收尾还原到进场时的样子）。

**偏离 17（第二轮 · 阶段 R2b）— 场景 5 的隐藏竞态：等 `htmlClass` 不够，要等远端命名空间装载完。**

本轮复现了一次场景 5 随机红：它清掉 localStorage 后重载，只等 `htmlClass` 出现就取 `prefs.hoverRestore`，
而 `Store` 的远端命名空间是**异步**装载的（`onRemoteLoaded` 里才 `state = loadState()`）——
引导期会先按默认值渲染出 htmlClass，于是读到的是默认值，表现为随机失败。

修法：把前置条件收紧为「`Store._remoteLoaded === true` 且 htmlClass 已出」。
**这不是放宽断言**（期望值一个没改），而是把测试自己的计时器修准。
