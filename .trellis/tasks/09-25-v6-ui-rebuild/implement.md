...
### R2b — 补齐新控件类型 + 三项「自建 DOM」设置进 schema ⬜
- [ ] 新增 `SviControls.colorList`（色卡列表增删）与 `SviControls.listEditor`（结构化列表增删）
- [ ] 面板 `buildShieldSection` 的色卡区 / 元素规则编辑器改用新工厂（DOM 结构随之变化 →
      撞到的 bench 断言按新结构改写并在偏离记录里说明）
- [ ] schema 补 `shieldColors` / `elementRules` / `bgReplace` 三项（`kind: 'colorList' / 'listEditor' / 'toggle'`）
- [ ] `test.js` 例外表删掉这 3 条（表自带断言：已上页面的键留在表里即变红）
- [ ] options 渲染层 `KINDS` 增加两种 kind 的实现（不认识的 kind 必须显式报错，不许静默跳过）
- [ ] 判据：options 页项数从 91 增到 **94**，且 `test-extension.js` 场景 7 与真源逐键比对通过
