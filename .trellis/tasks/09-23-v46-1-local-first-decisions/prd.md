...
- [ ] `research/` 落一份二分报告：引入版本（commit hash）、机制说明、复现步骤
- [ ] 限速（CDP `Network.emulateNetworkConditions` 或等价）下首屏可见媒体 ≤500ms 完成判定，探针带时间戳
- [ ] 断网（`offline`）下稳态决策与联网态一致（探针对比 JSON 摘要）
- [ ] 代码审查 + 单测证明：「未 complete 就跳过」不再是主路径（保留增强通道需单测断言降级行为）
- [ ] 运行时联网路径收敛：`gmFetchText` 不再被任何非用户显式动作触发（grep + 单测断言）
- [ ] 5 次重载 soak：稳态决策映射一致（沿用 `dev/probe-soak.js` 思路）
- [ ] 四绿门禁全绿
