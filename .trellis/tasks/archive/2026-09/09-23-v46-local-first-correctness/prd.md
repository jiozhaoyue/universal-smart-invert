...
> 其勾选**保持为空**（用户 2026-09-24 裁决：阈值 500ms 不放宽）。
...
- [ ] 弱网首屏：可见媒体在 **≤500ms** 内完成判定与落点（探针给出时间戳证据）
      → **未达阈值，勾选保持为空**：实测 **560ms**（归档期）/ **569ms**（2026-09-24 复测），超出 ≤500ms 阈值 60~69ms。
      2026-09-24 追加分解证据（`dev/probe-latency-breakdown.js`，同钟观测）：
      **插件可归因延迟仅 77ms**（body 就绪 → 首个决策落点；v4.6.1 事件驱动修复后进一步降至 44~49ms）；
      其余 ~490ms 全部是「取回 HTML 文档本身」的网络 RTT（探针注入 400ms）+ 导航开销，
      在插件获得 DOM 之前，插件不可能运行。
      修复前同类场景：首个决策 956ms、最慢一张 18s 内无结论 → 改善幅度 **956ms→560ms**。
      **裁决（用户，2026-09-24）**：**阈值保持 500ms 不放宽**，本项如实记为「显著改善但未达标」
      的已知遗留项，不调整验收标准；后续若继续优化属新任务范围（需另开任务，收益递减）。
