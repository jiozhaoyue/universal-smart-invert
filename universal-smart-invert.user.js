...
// @version      4.3.0
// @description  全网通用智能视频与图片反色脚本 (v4.3, AGPL-3.0 开源)。新增: 防闪光守卫 (深色站点文档起步即黑, 插件端 document_start), 元素规则「局部改色」(黑白同存只改浅色部分, 桶引擎路径), 反色纠错网 (深暗图绝不自动反色 + 像素反色决策限时复检自愈); 保留动态主题/定时模式/字体可读性等全部能力。
// @description:zh-CN 全网通用智能视频与图片反色脚本 (v4.3, AGPL-3.0 开源)。新增: 防闪光守卫、元素级局部改色 (只改浅色部分)、反色纠错网 (深暗图不误反 + 限时复检自愈); 保留全部既有能力。
// @description:en Universal smart video and image invert userscript (v4.3, AGPL-3.0 licensed). New: flash-black guard (document-start dark for dynamic-theme sites, extension now document_start), element-rule "recolor" (partial: only the light parts of a black-and-white element, bucket path), and an inversion sanity net (dark-dominant images never auto-inverted + bounded re-validation self-heal). All prior capabilities retained.
// @author       jiozhaoyue
